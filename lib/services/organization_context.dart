...
class OrganizationContext {
  ...
  static Future<void> initialize({String? specificOrgId}) async {
    try {
      ...
      _logger.e('❌ Failed to initialize organization context',
  ...
  static Future<void> setCurrentOrganization(String orgId) async {
    try {
      ...
      _logger.e('❌ Failed to set organization context',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  static Future<String> _getUserPrimaryOrganization(String userId) async {
    ...
    final orgsQuery = await _firestore
    ...
    try {
      ...
      if (subscriptionQuery.docs.isNotEmpty) {
        _currentSubscription =
            Subscription.fromFirestore(subscriptionQuery.docs.first);
  ...
  static bool hasPermission(String permission) {
    final permissions =
        _userPermissions?['permissions'] as List<dynamic>? ?? [];
